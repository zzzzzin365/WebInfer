/**
 * WebInfer - transformers.js Adapter Backend
 *
 * Wraps transformers.js (by Hugging Face) as an inference backend, giving
 * users access to 1000+ HuggingFace models while adding WebInfer's
 * orchestration layer (scheduling, caching, memory management, workers).
 *
 * @example
 * ```typescript
 * import { useTransformersBackend } from 'webinfer';
 * import { pipeline as tfPipeline } from '@xenova/transformers';
 *
 * // Register the adapter
 * useTransformersBackend();
 *
 * // Now use WebInfer pipeline API — inference delegates to transformers.js
 * const classifier = await pipeline('text-classification', {
 *   model: 'Xenova/distilbert-base-uncased-finetuned-sst-2-english',
 * });
 *
 * // WebInfer handles scheduling, batching, memory, caching
...
export class TransformersAdapterRuntime {
    ...
    async initialize() {
        if (!adapterOptions?.pipelineFactory) {
            throw new WebInferError('TransformersAdapterRuntime requires a pipelineFactory. ' +
