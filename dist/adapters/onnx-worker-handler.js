...
export function createONNXWorkerHandler(send) {
        ...
        tail = tail.then(async () => {
            try {
                ...
                switch (message.type) {
                    ...
                    case 'load': {
                        if (!message.data)
                            throw new Error('Model data required');
                    ...
                    case 'run': {
                        ...
                        if (!model)
                            throw new Error('Worker model not found');
                        const inputs = new Map(await Promise.all((message.inputs ?? []).map(async ([name, tensor]) => [name, await deserializeTensor(tensor)])));
                        try {
                            const outputs = await runtime.runNamed(model, inputs);
                            try {
                                const serialized = outputs.map(serializeTensor);
                                value = serialized;
                                transfers = serialized.map(t => t.data);
                            }
                            finally {
                    ...
                    case 'unload':
                        models.get(message.modelId)?.dispose();
                        models.delete(message.modelId);
                        break;
                    case 'dispose':
...
//# sourceMappingURL=onnx-worker-handler.js.map