  ...
  if (cache && !forceDownload) {
    ...
    if (cached) {
      ...
      if (isHtmlOrText || cached.byteLength < 1024) {
        console.warn(`[WebInfer] Cached model for ${url} appears corrupt (${cached.byteLength} bytes, first byte 0x${firstByte?.toString(16)}). Evicting and re-downloading.`);
        await modelCache.deleteModel(url);
      } else {
    ...
    ModelCache2 = class {
        ...
        try {
          ...
          if (this.isQuotaError(err)) {
            ...
            try {
              ...
              console.warn("[WebInfer] IndexedDB quota exceeded even after eviction; skipping cache.");
            }
          } else {
            throw err;
          }
        ...
        try {
          ...
          if (this.isQuotaError(err)) {
            ...
            try {
              ...
              console.warn("[WebInfer] IndexedDB quota exceeded even after eviction; skipping cache for chunk.");
            }
          } else {
            throw err;
          }
      ...
      async saveDownloadState(state) {
        try {
          ...
          if (this.isQuotaError(err)) {
            console.warn("[WebInfer] IndexedDB quota exceeded saving download state; resume may not work.");
          } else {
            throw err;
          }
        }
...
var InferenceScheduler = class {
  ...
  schedule(modelId, executor, priority = "normal") {
    ...
    if (this.isCircuitOpen(modelId)) {
      throw new WebInferError(`Circuit breaker is open for model ${modelId} \u2014 too many consecutive failures. Retry after ${this.options.circuitBreakerResetTimeout}ms.`, ErrorCodes.INFERENCE_FAILED, { modelId });
    }
...
async function registerPlugin(plugin) {
  if (registeredPlugins.has(plugin.name)) {
    console.warn(`[WebInfer] Plugin "${plugin.name}" is already registered \u2014 skipping.`);
    return;
  }
  if (plugin.setup) {
    await plugin.setup();
...
var TransformersAdapterRuntime = class {
  ...
  async initialize() {
    if (!adapterOptions?.pipelineFactory) {
      throw new WebInferError("TransformersAdapterRuntime requires a pipelineFactory. Call useTransformersBackend({ pipelineFactory }) first.", ErrorCodes.RUNTIME_INIT_FAILED);
    }
  }
  async loadModel(modelData, options = {}) {
  ...
  async loadPipeline(task, model, pipelineOptions) {
    if (!adapterOptions?.pipelineFactory) {
      throw new WebInferError("Adapter not initialised", ErrorCodes.RUNTIME_NOT_INITIALIZED);
    }
   ...
   * The result is returned as a single WebInferTensor wrapping the JSON-encoded output
  ...
  async run(model, inputs) {
    ...
    return [new WebInferTensor(resultArray, [resultArray.length], "float32")];
  }
  /**
...
var TextClassificationPipeline = class extends BasePipeline {
  ...
  async preprocess(input) {
    ...
    const inputIds = new WebInferTensor(BigInt64Array.from(encoded.inputIds.map((id) => BigInt(id))), [1, encoded.inputIds.length], "int64");
...
var TextGenerationPipeline = class extends BasePipeline {
  ...
  async generateNextToken(inputIds, temperature, topK, topP, repetitionPenalty, doSample) {
    ...
    inputs.set("input_ids", new WebInferTensor(BigInt64Array.from(inputIds.map((id) => BigInt(id))), [1, seqLen], "int64"));
    inputs.set("attention_mask", new WebInferTensor(BigInt64Array.from(inputIds.map(() => BigInt(1))), [1, seqLen], "int64"));
    inputs.set("position_ids", new WebInferTensor(BigInt64Array.from(Array.from({ length: seqLen }, (_, i) => BigInt(i))), [1, seqLen], "int64"));
    const numLayers = 22;
    const numKVHeads = 4;
    const headDim = 64;
...
var QuestionAnsweringPipeline = class extends BasePipeline {
  ...
  async answerQuestion(input, options) {
    ...
    const startProbs = softmax(new WebInferTensor(new Float32Array(startLogits), [seqLen], "float32")).toFloat32Array();
    const endProbs = softmax(new WebInferTensor(new Float32Array(endLogits), [seqLen], "float32")).toFloat32Array();
    let bestStartIdx = 0;
    let bestEndIdx = 0;
    let bestScore = 0;
...
var ImageSegmentationPipeline = class extends BasePipeline {
  ...
  preprocessImage(imageData) {
    ...
    return {
      tensor: new WebInferTensor(tensorData, [1, 3, this.inputSize, this.inputSize], "float32"),
  ...
  prepareDecoderInputs(points, boxes) {
    ...
    const inputs = /* @__PURE__ */ new Map();
    inputs.set("input_points", new WebInferTensor(new Float32Array(allPoints), [1, 1, numPoints, 2], "float32"));
    inputs.set("input_labels", new WebInferTensor(BigInt64Array.from(allLabels.map((l) => BigInt(l))), [1, 1, numPoints], "int64"));
    return inputs;
  }
  /**
...
function parallel(stages) {
  if (stages.length === 0) {
    throw new Error("[WebInfer] parallel() requires at least one stage");
  }
  let pipelineInstances = null;
  async function ensureInitialised() {
    if (pipelineInstances)
...
var WebInferDebugger = class {
  ...
  defaultLogger(level, message, data) {
    ...
    const prefix = `[WebInfer ${timestamp}] [${level.toUpperCase()}]`;
    switch (level) {
      case "debug":
...
<head>
  ...
  <title>WebInfer Performance Dashboard</title>
  <style>
    :root {
      --bg-primary: #0d1117;
      --bg-secondary: #161b22;
  ...
  <div class="dashboard">
    <header>
      <h1>
        ...
        WebInfer Performance Dashboard
      </h1>
      <div class="status">
    ...
    <footer>
      Generated at ${(/* @__PURE__ */ new Date()).toLocaleString()} | WebInfer Performance Monitor
    </footer>
  </div>
</body>
</html>
...
function generateAsciiDashboard(monitor) {
  ...
  const lines = [
    "\u2554\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2557",
    "\u2551             WebInfer Performance Monitor Dashboard                   \u2551",
    "\u2560\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2563",
    "\u2551                                                                          \u2551",
