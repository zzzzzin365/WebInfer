...
export interface ForegroundPrompt {
    x: number;
    y: number;
    label: 0 | 1;
}
...
export interface IntentClassifier {
    classify(request: GenerationRequest, context: ExecutionContext): Promise<Array<{
        route: GenerationRoute;
        score: number;
    }>>;
}
export interface AssetClassifier {
    version: string;
    classify(image: PixelImage, context: ExecutionContext): Promise<{
        category: string;
        confidence: number;
    }>;
}
...
export interface GenerationService {
    textTo3D(input: {
        text: string;
    }, context: ExecutionContext): Promise<GenerationJob>;
    imageTo3D(input: {
        text: string;
        imageUrl: string;
    }, context: ExecutionContext): Promise<GenerationJob>;
