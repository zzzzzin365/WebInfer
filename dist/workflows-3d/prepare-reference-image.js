...
export async function prepareReferenceImage(segmenter, original, prompts, context = {}, maxDimension = 1024) {
    const execution = { ...context, priority: context.priority ?? 'high' };
    throwIfAborted(execution.signal);
    try {
        const { width, height, data } = original;
        if (!Number.isInteger(width) || !Number.isInteger(height) || width <= 0 || height <= 0 ||
            data.length !== width * height * 4 || !Number.isInteger(maxDimension) || maxDimension <= 0)
            throw new Error('Invalid image dimensions');
        if (!prompts.some(p => p.label === 1) || prompts.some(p => !Number.isFinite(p.x) || !Number.isFinite(p.y) ||
            p.x < 0 || p.x > 1 || p.y < 0 || p.y > 1))
            throw new Error('A foreground point is required');
        ...
        if (!Number.isInteger(mask.width) || !Number.isInteger(mask.height) || mask.width <= 0 || mask.height <= 0 ||
            ...
            throw new Error('Invalid foreground mask');
        const alphaAt = (x, y) => mask.data[Math.floor(y * mask.height / height) * mask.width + Math.floor(x * mask.width / width)];
...
//# sourceMappingURL=prepare-reference-image.js.map