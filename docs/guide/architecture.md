...
WebInfer does not lock you into a single inference engine. The `Runtime` interface is intentionally minimal:
...
Third-party plugins can register new pipeline tasks, backends, and middleware without modifying WebInfer source. See [Plugin System](/guide/plugins).
