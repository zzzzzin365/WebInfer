...
WebInfer supports plugins that register custom pipelines, backends, and middleware at runtime.

## Creating a Plugin

```typescript
import { registerPlugin, type WebInferPlugin } from 'webinfer';

const myPlugin: WebInferPlugin = {
...
```typescript
import { listPlugins, unregisterPlugin } from 'webinfer';

console.log(listPlugins());
// [{ name: 'webinfer-plugin-whisper', version: '1.0.0' }]

unregisterPlugin('webinfer-plugin-whisper');
```
