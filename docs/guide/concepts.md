...
Pipeline 是使用 WebInfer 的主要方式。每个 Pipeline 封装了：
...
```typescript
import { tensor, WebInferTensor } from 'webinfer';
...
```typescript
import { getScheduler, TaskPriority } from 'webinfer';
...
```typescript
import { getMemoryManager } from 'webinfer';
...
```typescript
import { isModelCached, getCachedModel, clearModelCache } from 'webinfer';
...
```typescript
import { Tokenizer } from 'webinfer';
