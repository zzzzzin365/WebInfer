...
import { WebInferTensor, softmax } from '../../src/core/tensor';
 ...
 */
function createMockOutputs(shapes: { shape: number[]; fill?: number }[]): WebInferTensor[] {
  return shapes.map(({ shape, fill }) => {
    ...
    return new WebInferTensor(data, shape, 'float32');
  });
}
...
describe('TextClassificationPipeline', () => {
  it('should postprocess logits into label + score', () => {
    ...
    const logits = new WebInferTensor(new Float32Array([1.5, -0.5]), [1, 2], 'float32');
  ...
  it('should handle batch of logits', () => {
    ...
    const texts = [
      new WebInferTensor(new Float32Array([-2.0, 3.0]), [1, 2], 'float32'), // POSITIVE
      new WebInferTensor(new Float32Array([3.0, -2.0]), [1, 2], 'float32'), // NEGATIVE
      new WebInferTensor(new Float32Array([0.1, 0.1]), [1, 2], 'float32'),  // ~neutral
    ];
  ...
  it('should handle multi-class logits', () => {
    const logits = new WebInferTensor(
...
describe('FeatureExtractionPipeline', () => {
  it('should produce embeddings of correct dimension', () => {
    ...
    const hiddenStates = new WebInferTensor(data, [1, seqLen, hiddenDim], 'float32');
...
describe('ImageClassificationPipeline', () => {
  it('should postprocess logits to label', () => {
    ...
    const tensor = new WebInferTensor(logits, [1, 1000], 'float32');
    const probs = softmax(tensor, -1) as WebInferTensor;
    const arr = probs.toFloat32Array();
