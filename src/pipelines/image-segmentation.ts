/**
 * WebInfer - Image Segmentation Pipeline
 * 
...
> {
  ...
  private imageEmbedding: WebInferTensor | null = null;
  private imagePositionalEmbedding: WebInferTensor | null = null;
   ...
   */
  async setImage(image: ImageInput, options?: PipelineOptions): Promise<void> {
    ...
    if (this.encoderModel) {
      ...
      this.imageEmbedding = outputs[0] as WebInferTensor;
      this.imagePositionalEmbedding = outputs[1] as WebInferTensor;
  ...
  async segment(options: ImageSegmentationOptions = {}): Promise<ImageSegmentationResult> {
    ...
    const outputs = await this.inference.runInferenceNamed(this.decoderModel, decoderInputs, options).finally(() => {
      for (const t of decoderInputs.values()) {
        if (t !== this.imageEmbedding && t !== this.imagePositionalEmbedding) t.dispose();
      }
    });

    // SAM decoder outputs: [masks, iou_predictions]
    const masks = outputs[0] as WebInferTensor;
    const scores = outputs[1] as WebInferTensor;
  ...
  private prepareDecoderInputs(
    ...
    const inputs = new Map<string, WebInferTensor>();

    // input_points: [1, 1, num_points, 2] - SlimSAM format (float32)
    inputs.set('input_points', new WebInferTensor(
    ...
    inputs.set('input_labels', new WebInferTensor(
  ...
  private postprocessMasks(
    masks: WebInferTensor,
    scores: WebInferTensor,
    threshold: number,
    returnAllMasks: boolean
  ): ImageSegmentationResult {
   ...
   */
  protected override async preprocess(input: ImageInput): Promise<WebInferTensor[]> {
