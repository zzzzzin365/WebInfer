...
export class TextGenerationPipeline extends BasePipeline<string | string[], TextGenerationResult | TextGenerationResult[]> {
  ...
  private async generateNextToken(
    ...
    // input_ids: [1, seq_len]
    inputs.set('input_ids', new WebInferTensor(
    ...
    inputs.set('attention_mask', new WebInferTensor(
    ...
    inputs.set('position_ids', new WebInferTensor(
    ...
    for (let i = 0; i < numLayers; i++) {
      ...
      inputs.set(`past_key_values.${i}.key`, new WebInferTensor(
        new Float32Array(0),
        [1, numKVHeads, 0, headDim],
        'float32'
      ));
      // past_key_values.{i}.value: [batch, num_kv_heads, 0, head_dim]
      inputs.set(`past_key_values.${i}.value`, new WebInferTensor(
        new Float32Array(0),
        [1, numKVHeads, 0, headDim],
        'float32'
      ));
