/**
 * WebInfer - Plugin System
 *
 * Register custom pipelines, backends, and middleware via plugins.
 *
 * @example
 * ```typescript
 * import { registerPlugin } from 'webinfer';
 *
 * registerPlugin({
 *   name: 'webinfer-plugin-whisper',
 *   version: '1.0.0',
 *   pipelines: {
 *     'whisper-transcribe': {
 ...
 * Plugin definition.
 */
export interface WebInferPlugin {
  /** Unique plugin name (e.g. 'webinfer-plugin-whisper') */
  name: string;
...
const registeredPlugins = new Map<string, WebInferPlugin>();
 ...
 */
export async function registerPlugin(plugin: WebInferPlugin): Promise<void> {
